...
STARTER_PROMPTS = [
    "😔 Aku lagi stres banget",
    "😰 Aku sering cemas belakangan ini",
    "😴 Aku susah tidur terus",
    "💭 Aku butuh teman ngobrol",
    "🧠 Ajari aku teknik relaksasi",
...
def get_weekly_insight():
    ...
    if avg >= 4.0:
        return f"Minggu yang luar biasa! Rata-rata mood {avg:.1f}/5 💚"
    if avg >= 3.0:
        return f"Minggu cukup baik. Rata-rata mood {avg:.1f}/5 🌿"
    if avg >= 2.0:
        return f"Minggu yang lumayan berat. Rata-rata {avg:.1f}/5 💙"
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='"
        "background:linear-gradient(135deg,#1A5C52,#5C7A6B);"
        "padding:20px 20px 16px;"
        "position:relative; overflow:hidden;'>"
        "<div style='display:flex;align-items:center;gap:12px;position:relative;'>"
        "<div style='width:42px;height:42px;background:rgba(255,255,255,0.15);"
        "border-radius:50%;display:flex;align-items:center;justify-content:center;"
        "font-size:20px;border:2px solid rgba(255,255,255,0.25);'>🌿</div>"
        "<div>"
    ...
    # Mood Tracker
    st.markdown("<div style='padding:16px 20px;border-bottom:1px solid rgba(92,122,107,0.15);'>", unsafe_allow_html=True)
    ...
    if st.session_state.mood_history:
        ...
        st.markdown(
            ...
            f"<div class='stat-card'><div class='stat-val'>{avg:.1f}</div><div class='stat-lbl'>rata-rata</div></div>"
            f"<div class='stat-card'><div class='stat-val'>{len(tanggals)}</div><div class='stat-lbl'>hari</div></div>"
            f"<div class='stat-card'><div class='stat-val'>{today_emoji}</div><div class='stat-lbl'>hari ini</div></div>"
            f"</div>",
            unsafe_allow_html=True
        )
        ...
        ax.plot(range(len(tanggals)), levels, "o-",
                ...
                markeredgecolor="#1A5C52", zorder=3)
    ...
    st.markdown(
        ...
        "⚠️ <strong>MindEase</strong> adalah alat bantu edukasi, bukan pengganti psikolog profesional.<br><br>"
...
st.markdown(
    "<div style='"
    "background:linear-gradient(135deg,#1A5C52 0%,#5C7A6B 100%);"
    "padding:22px 36px 18px;"
    "position:relative;overflow:hidden;'>"
    "<div style='display:flex;align-items:center;gap:16px;position:relative;'>"
    "<div style='width:50px;height:50px;background:rgba(255,255,255,0.15);"
    "border-radius:50%;display:flex;align-items:center;justify-content:center;"
    "font-size:24px;border:2px solid rgba(255,255,255,0.25);flex-shrink:0;'>🌿</div>"
    "<div>"
    "<div style='font-family:Fraunces,serif;font-style:italic;font-size:24px;"
    ...
    "padding:4px 12px;font-size:11px;'>💬 Curhat bebas</span>"
    "<span style='background:rgba(255,255,255,0.12);color:rgba(255,255,255,0.82);"
    "border:1px solid rgba(255,255,255,0.2);border-radius:999px;"
    "padding:4px 12px;font-size:11px;'>📚 Berbasis RAG</span>"
    "</div></div></div>",
    unsafe_allow_html=True
)

# Welcome screen atau chat
if not st.session_state.messages:
    ...
    st.markdown(
        ...
        "</div>"
        "<div style='font-size:12px;font-weight:600;color:#5C7A6B;"
        "text-transform:uppercase;letter-spacing:0.05em;margin-bottom:12px;'>"
        "Mulai dari sini:</div>"
        f"<div class='starter-grid'>{chips}</div>"
        "</div>",
        unsafe_allow_html=True
    )
else:
...
if prompt := st.chat_input("Ceritakan apa yang kamu rasakan..."):
    ...
    st.session_state.messages.append({"role": "user", "content": prompt, "time": ts_now})
